  ...
  "rules": {
    "app_data": {
      ...
      "quizzes": {
        ...
        ".write": "auth !== null && root.child('app_data/users').child(auth.uid).child('admin').exists()",
        "$quizId": {
          ".validate": "newData.hasChildren(['title', 'description', 'level', 'category', 'timeLimit', 'questions'])",
          "title": {
            ".validate": "newData.isString() && newData.val().length > 0"
          },
          "description": {
          ...
          "category": {
            ".validate": "newData.isString() && newData.val().matches(/^(vocabulary|grammar|kanji|listening)$/)"
          },
          "timeLimit": {
            ".validate": "newData.isNumber() && newData.val() > 0"
          },
          "questions": {
            "$questionId": {
              ".validate": "newData.hasChildren(['type', 'question', 'correctAnswer', 'points'])",
              "type": {
                ".validate": "newData.isString() && newData.val().matches(/^(MULTIPLE_CHOICE|TRUE_FALSE|FILL_BLANK|MATCHING|AUDIO_CHOICE)$/)"
              },
              "question": {
                ".validate": "newData.isString() && newData.val().length > 0"
              },
      ...
      "user_quiz_progress": {
        "$uid": {
          ...
          "completed_quizzes": {
            "$quizId": {
              ".validate": "newData.hasChildren(['score', 'completedAt', 'timeTaken'])",
              "score": {
                ".validate": "newData.isNumber() && newData.val() >= 0"
              },
              "completedAt": {
              ...
              "answers": {
                "$questionId": {
                  ".validate": "newData.hasChildren(['selectedAnswer', 'isCorrect', 'timeSpent'])",
                  "selectedAnswer": {
          ...
          "quiz_statistics": {
            ".validate": "newData.hasChildren(['totalQuizzesTaken', 'averageScore', 'bestScore', 'totalTimePlayed'])",
            "totalQuizzesTaken": {
              ".validate": "newData.isNumber() && newData.val() >= 0"
            },
            "averageScore": {
            ...
            "categoryProgress": {
              ".validate": "newData.hasChildren(['vocabulary', 'grammar', 'kanji', 'listening'])",
              "$category": {
                ".validate": "newData.isNumber() && newData.val() >= 0 && newData.val() <= 100"
              }
            },
            "levelProgress": {
              ".validate": "newData.hasChildren(['N5', 'N4', 'N3', 'N2', 'N1'])",
              "$level": {
                ".validate": "newData.isNumber() && newData.val() >= 0 && newData.val() <= 100"
              }
            }
